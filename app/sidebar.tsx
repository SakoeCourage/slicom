...
export default function Sidebar({ toggleSidebar }: { toggleSidebar: () => void }) {
    return (
        <div className=" h-screen overflow-hidden w-full lg:max-w-[17rem] bg-[#F6F8FC] ">
            <nav className=" basis-[var(--header-height)] w-full  text-white py-1 border-b-2 ">
                <nav className="text-4xl font-bold flex flex-col justify-start items-start px-5 mb-1 space-x-3">
                    <Image className=" bg-blend-multiply" src="/images/slicomlogo.png" alt="slico-icon" width={150} height={150} quality={50} />
                </nav>
            </nav>
            <ul className="overflow-y-scroll hiddenscroll w-full basis-auto flex flex-col gap-5 h-full min-h-max list-none py-4 pb-[var(--header-height)]">
                {sideBarSections.map((section, i) => <li key={i} className="w-full flex flex-col gap-3 text-sm tracking-tight capitalize whitespace-nowrap"  >
                    <nav className=" flex items-center  px-6 py-2  uppercase tracking-wide text-[#B1B3B5] text-xs font-medium">{section.sectionName}</nav>
                    <ul className=" w-full basis-auto flex flex-col gap-1 h-full min-h-max list-none">
